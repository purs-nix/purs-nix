    ...
    purs-nix-flake.inputs.utils.apply-systems
      ...
      (
        ...
        let
          ...
          switches =
            mapAttrs (n: v: (a: l.warnIf (!a) "switches.${n} == false" a) (!minimal && v))
              {
          ...
          ps2 = purs {
            dependencies =
              ...
              with ps-pkgs; [
                ...
                (l.recursiveUpdate test.prelude {
          ...
          package-tests = import ./packages.nix (
            { inherit l p switches; } // (purs-nix-flake { inherit system; })
          );
        ...
        in
        {
          apps.default = {
            ...
            program =
              let
                path = package-tests."compiled packages bucket 1" + /cache-db.json;
              in
              (p.writeScript "inspect-packages" "${p.jq}/bin/jq . ${path} | less").outPath;
          };

          checks =
            l.deepSeq switches mapAttrs
              (
                n: v:
                p.runCommand n { } ''
                  ${v}
              ...
              )
              {
                "compiler flags" =
                  ...
                  in
                  make-test "codegen" "" (_: "ls ${output}/Main/corefn.json")
                  +
                  ...
                  +
                    ...
                    make-test "no-prefix" "echo $(head -n 1 ${output}/Main/index.js | grep //)" (
                      i: "[[ -z ${i} ]]"
                    );
                ...
                "custom node package" =
                  ...
                  in
                  make-test "node version" (run-app-custom { inherit nodejs; } "Node") (
                ...
                "custom purescript package" =
                  ...
                  in
                  make-test "purescript version" "head -n 1 ${output}/Main/index.js" (
                    i:
                    ''[[ ${i} == "// Generated by purs version ${
                      substring 1 99 purescript.version
                    }" ]]''
                  );

                "main app output" =
                  make-test "expected output" "${run-app "Main"} argument"
                    expected-output;

                "main script output head" =
                  make-test "expected output" "head -n 1 <(${ps.script { }})"
                    (i: "[[ ${i} == *-Main-script ]]");

                "main script output tail" =
                  make-test "expected output" "tail -n +2 <(${ps.script { }} argument)"
                    (i: ''[[ ${i} == "${expected-output-tail}" ]]'');

                "main output murmur3" =
                  make-test "expected output" "${ps2.app { name = "_"; }}/bin/_"
                    (i: "[[ ${i} == 1945310157 ]]");

                "Prelude has foreign dependencies (build)" = make-test "file exists" "" (
                  _: "ls ${ps2.output { }}/Prelude/foreign"
                );

                "app minification" =
                  let
                    a = args: ps.app ({ name = "_"; } // args);
                  in
                  ''
                    a=$(wc -c < ${a { }}/bin/_)
                ...
                "ps.dependencies" =
                  make-test "expected number" "echo ${toString (length ps.dependencies)}"
                    (i: "[[ ${i} == 59 ]]");

                "test run defaults" = make-test "expected output" "${ps.test.run { }}" (
                  i: "[[ ${i} == testing ]]"
                );
                ...
                "no test app" =
                  make-test "expected output"
                    "${
                      (ps-custom { test = "nonexistent"; }).app { name = "test run"; }
              ...
              mapAttrs
                    ...
                    src =
                      filterSource
                        (
                          ...
                          (
                            type == "directory"
                            && any (s: (match ".*/${s}" path != null) || l.hasInfix "/${s}/" path) (
                              args.srcs or [
                                "src"
                                "src2"
                              ]
                    ...
                    checkPhase =
                      (
                        if less then
                          ...
                          make-test "purs-nix package-info prelude" "${command} package-info prelude" (
                            i: ''
                              info="name:    prelude
                          ...
                          +
                            ...
                            make-test "purs-nix package-info effect" "${command} package-info effect" (
                              i: ''
                                info="name:    effect
                                version: override-test
                                repo:    https://github.com/purs-nix/test-packages.git
                          ...
                          +
                            ...
                            make-test "purs-nix packages" "${command} packages" (
                              i: ''
                                packages="aff: 7.1.0
                          ...
                          +
                            ...
                            make-test "purs-nix bower" "" (_: "${command} bower")
                          +

                            make-test "purs-nix test" "${command} test" (i: "[[ ${i} == testing ]]")
                      )
                      +

                        make-test "purs-nix bundle" "" (_: "${command} bundle")
                      +

                        make-test "purs-nix run" "${command} run argument" run-output
                      +

                        make-test "purs-nix docs" "" (_: "${command} docs")
                      +
                      ...
                      +
                        ...
                        (
                          if switches.repl then
                            let
                              repl = "echo :q | HOME=. ${command} repl";
                            in
                            make-test "purs-nix repl" "" (_: repl)
                            +

                              make-test "purs-nix repl: create .purs-repl" "cat .purs-repl" (
                                i: ''[[ ${i} == "import Prelude" ]]''
                              )
                            +
                ...
                )
                {
                  "purs-nix command defaults" = {
                    test =
                      command:
                      make-test "check output name" "" (_: "ls ${output-default}")
                      +

                        make-test "purs-nix srcs" "${command} srcs" (
                          i: "${purs-nix.purescript}/bin/purs compile ${i}"
                        )
                      +

                        make-test "main.js exists" "" (_: "ls main.js")
                      +
                  ...
                  "purs-nix command configured" =
                    ...
                    in
                    {
                      ...
                      test =
                        command:
                        make-test "list flags work" "echo ${command}" (
                          _:
                          ''cat ${command} | grep -- "--external:'1' --external:\"2\" --external:\$three --external:'4'"''
                        )
                        +

                          make-test "custom-named output exists" "" (_: "ls ${outfile}")
                        +

                          make-test "bower.json is what we expect" "diff bower.json ${./bower.json}" (
                            i: "[[ -z ${i} ]]"
                          )
                        +
                        ...
                        +
                          ...
                          make-test ''"output" does not exist'' "ls" (_: "[[ ! -e output ]]");
                    };
                ...
                let
                  overlay =
                    (purs-nix.build {
                      ...
                      null;
                in
                assert overlay ? "purs-nix.build-test" && length (attrValues overlay) == 1;
